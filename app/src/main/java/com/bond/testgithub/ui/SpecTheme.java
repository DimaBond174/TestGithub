package com.bond.testgithub.ui;
/*
...
import com.bond.testgithub.objs.ImageFetcher;
...
public class SpecTheme {
    ...
    public static final int PBlueColorA           = 0x992e7d32;
    ...
    public static volatile int dpBigAva = 174; //dp Большая аватарка
    public static volatile int dpBigAvaHalf = 87; //dp Большая аватарка
    public static volatile int dpProgress = 80; //dp круг прогресс бара
    public static volatile int dpProgressHalf = 40; //dp пол круга
    public static volatile int dpTextPadding     = 72; //dp Начало текста после иконок
    public static volatile int dpBottomButtons     = 60; //dp Начало текста после иконок
    ...
    * это блок адаптированных цветов для интерфейса */
    ...
    private static final int BigAvaSize = 174; //dp Макс размер клавы
    private static final int ProgressSize = 90; //dp Макс размер клавы
    private static final int TextPadding     = 72; //dp Начало текста после иконок
    private static final int BottomButtonsSize     = 60; //dp Начало текста после иконок
    ...
    public static IActivity iActivity = null;
    public static Resources resources = null;
    public static final ImageFetcher imageFetcher = new ImageFetcher();
...
//    public static Drawable fileIcon         = null; //Иконка файла
    public static Drawable github_icon      = null;
  ...
  //Блокиратор множественного нажатия (когда много виджетов хватаются за одно касание)
    ...
    public static void applyMetrics(Context dialogCtx)  {
        ...
        iActivity =  (IActivity)dialogCtx;
        github_icon = ContextCompat.getDrawable(context
            , R.drawable.github);
        imageFetcher.start();
...
//        cancelWrong.setColorFilter(new LightingColorFilter( 0, PRedBADColor));
        ...
        SpecTheme.dpBigAva = (int) Math.ceil(SpecTheme.BigAvaSize * den);
        SpecTheme.dpBigAvaHalf = SpecTheme.dpBigAva >> 1;
        SpecTheme.dpProgress = (int) Math.ceil(SpecTheme.ProgressSize * den);
        SpecTheme.dpProgressHalf = SpecTheme.dpProgress >> 1;
        SpecTheme.dpTextPadding     = (int) Math.ceil(SpecTheme.TextPadding * den);
        SpecTheme.dpBottomButtons     = (int) Math.ceil(SpecTheme.BottomButtonsSize * den);
