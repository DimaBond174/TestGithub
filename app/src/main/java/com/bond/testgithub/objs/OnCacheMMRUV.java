 ...
 *  It used volatile landscape pointer
 *
 * Usage:
...
public class OnCacheMMRUV<K extends IKey, V>  {
  // Public Java Interface :
  /////////////////////////////////////////////////////////////////
  public OnCacheMMRUV(int  capacity, int  uselessness)  {
    l_capacity  =  capacity;
...
//      int  cmp  =  compare(p_key,  cur->fwdPtrH->key);
      ...
      if (cmp  <  0)  {
        ...
        prevHead.curHeight  =  landscape_l[128 + ++land_l_p];
        for (int  i  =  0; i  <=  2;  ++i)  {
...
//          cmp  =  compare(p_key,  cur->fwdPtrsL[h]->key);
      ...
      new_node.curHeight  =  landscape_l[128 + ++land_l_p];
...
//    int  cmp  =  compare(p_key,  cur->key);
    ...
    if (cmp  <  0)  {
      ...
      cur.curHeight  =  landscape_l[128 + ++land_l_p];
      for (int  i  =  0; i  <=  2;  ++i)  {
        if (cur.curHeight  >=  i)  {
